...
void GencodePreprocess()
{
	...
	char oneLine[600];
	while (fgets(oneLine, 600, fp) != NULL)
	{
		gencodeTotalLineNum++;
	}
...
void CreateHashTable()
{
	...
	char chrID[10];
	char location[10];
	char location_next[10];
	rounds++;
	char previousReadID[50];
	char currentReadID[50];

	printf("Round %d: CreateHashTable!\n", rounds);
	while (fgets(oneLine, 600, genomeFp) != NULL)
	{
		...
		genomeTotalLineNum++;
		readID = new char[50];
		chrIDLocation = new char[35];
		//printf("start to read..\n");
		sscanf(oneLine, "%s %*s %s %s %*s %*s %*s %s", readID, chrID, location, location_next);
		if (chrID[0]=='*')
                {
			delete readID;
		...
		else
		{
			...
			{
				hashTable[readID].push_back(chrIDLocation);
				//printf("chrIDLocation not found\n");
				toBeDeleted.push_back(readID);
				toBeDeleted.push_back(chrIDLocation);
			}
			else 
...
void ProcessGencode()
{
	...
	while (fgets(oneLine, 600, fp) != NULL)
	{
		...
		//printf("gencodeCurrentLineNum:%d\n", gencodeCurrentLineNum);
		//printf("%d\n", gencodeCurrentLineNum);
		...
		*pch = ' ';*/
		...
		int startInGenome_next = 0;
		int firstHalf_next = 0;
		int middle_next = 0;
		int secondHalf_next = 0;
		int startInGencode_next = 0;
		//printf("done initial\n");
		...
		*pch = ' ';
		...
		if (hashTable.find(readID) != hashTable.end())
		{
			...
			{
				unsigned int loop_end = junctions[(const char*)isophone].size();
				for (unsigned int i = 0; i < junctions[(const char*)isophone].size() ; i++ )
				{
					if (startInGencode + readLength - 1 <= junctions[(const char*)isophone][i][0])
					{
						startInGenome = junctions[(const char*)isophone][i][2] - (junctions[(const char*)isophone][i][0] - startInGencode);
            					break;
					}
					else if ((startInGencode + readLength - 1 > junctions[(const char*)isophone][i][0]) && (startInGencode <= junctions[(const char*)isophone][i][0]))
					{
						found2 = true;
					...
					{
						startInGenome = junctions[(const char*)isophone][i][3] + (startInGencode - junctions[(const char*)isophone][i][0]);
					}
				}
			}
			bool found3 = false;
			if (flag == '=')
			{
				startInGencode_next=eighthColumn;
        			unsigned int loop_end = junctions[(const char*)isophone].size();
      				for (unsigned int i = 0; i < junctions[(const char*)isophone].size() ; i++ )
       				{
       					if (startInGencode_next + readLength - 1 <= junctions[(const char*)isophone][i][0])
         				{
       						startInGenome_next = junctions[(const char*)isophone][i][2] - (junctions[(const char*)isophone][i][0] - startInGencode_next);
           					break;
       					}
      					else if ((startInGencode_next + readLength -1  > junctions[(const char*)isophone][i][0]) && (startInGencode_next <= junctions[(const char*)isophone][i][0]))
       					{
          					found3 = true;
           					firstHalf_next = junctions[(const char*)isophone][i][0] - startInGencode_next + 1;
        					middle_next = junctions[(const char*)isophone][i][3] - junctions[(const char*)isophone][i][2];
           					secondHalf_next = readLength - firstHalf_next;
           					startInGenome_next = junctions[(const char*)isophone][i][2] - firstHalf_next + 1;
           					break;
         				}
          				else if ((--loop_end == 0))
        				{		
          					startInGenome_next = junctions[(const char*)isophone][i][3] + (startInGencode_next - junctions[(const char*)isophone][i][0]);
         				}
      				}
			}
			...
			if (found2 == true || found3 == true)
			{
				...
				char *chrIDLocation = new char[35];
				sprintf(chrIDLocation, "%s:%d:%d", chrID, startInGenome, startInGenome_next);
				bool found = false;
				for (unsigned int i = 0; i < hashTable[readID].size(); i++ )
				{
					if (strcmp(hashTable[readID][i], chrIDLocation) == 0)
				...
				{
					hashTable[readID].push_back(chrIDLocation);
					char *oneEntry = new char[600];
                                        if (found2 == true)
					{
						sprintf(oneEntry, "%s\t%d\t%s\t%d\t%d\t%dM%dN%dM\t%c\t%d\t%d\t%s\t%s\t%s\t%s\t%s\n", readID, secondColumn, chrID, startInGenome, fifthColumn, firstHalf, middle, secondHalf, flag, startInGenome_next, ninthColumn, series1, series2, twelfthColumn, thirteenthColumn, lastColumn);
					}
					else
					{
						sprintf(oneEntry, "%s\t%d\t%s\t%d\t%d\t%s\t%c\t%d\t%d\t%s\t%s\t%s\t%s\t%s\n", readID, secondColumn, chrID, startInGenome, fifthColumn, CIGAR, flag, startInGenome_next, ninthColumn, series1, series2, twelfthColumn, thirteenthColumn, lastColumn);
					}
					toBeDeleted.push_back(chrIDLocation);
...
void ProcessGenome()
{
	...
	while (lineNum != 0)
	{
		if (fgets(oneLine, 600, genomeFp2) == NULL)
		{
